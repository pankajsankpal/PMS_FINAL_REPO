...
public class ManageProfile extends HttpServlet implements ConstantValues {
	...
	public ModelAndView addProfile(HttpServletRequest request, @RequestParam Map<String, String> r) throws Exception {
		...
		try {
			...
			/*
			 * Modified_date cannot be added initially hence null else error
			 * occurs during database insertion
			 */
			jobBean.setModified_date(null);
			...
			Date date = new Date();
			/**
			 * Below comments are trials for fixing the date accepting
			 * issue....ignore them
			 */
			...
			/*
			 * Since criteria_id is foreign key for company it has to be set by
			 * fetching it from criteriaBean
			 */
			...
			/*
			 * @author Nevil Dsouza code for notifications
			 */
			...
			// emailNotificationService.sendEmailNotification(receivers,
			// category, message);
	...
	public ModelAndView listProfile1() {
		Map<String, Object> model = new HashMap<String, Object>();
		// model.put("profiles",
		// prepareListofBean(manageProfileService.listProfile()));
		// return new ModelAndView("viewProfile", model);
		return new ModelAndView("CompaniesPage");
	}
	...
	public ModelAndView Companies(@RequestParam("companyname") String companyname) {
		...
		try {
			...
			for (CompanyBean cb : clist) {
				// System.out.println("Clist company name
				// "+cb.getCompany_name());
				// System.out.println("Company Name "+companyname);
				c = new CompanyBean();
				c = cb;
	...
	public ModelAndView test(@RequestParam("companyname") String companyname) {
		try {
			...
			// model = null;
			// 1. get company list using getCompany from company dao
			// 2. iterate company list and select a new company bean which
			// matches
			// with String companyname
			// 3. model.addObject() the new company bean
